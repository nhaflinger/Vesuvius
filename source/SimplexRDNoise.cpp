...
unsigned char srdperm[512] = { 151,160,137,91,90,15,
...
float SimplexRDNoise::SimplexRDNoise2D(float x, float y, float angle, float *dnoise_dx, float *dnoise_dy)
{
	...
	* Determine which simplex we are in. */
	...
	/* Wrap the integer indices at 256, to avoid indexing srdperm[] out of bounds */
	int ii = i % 256;
	int jj = j % 256;
	...
	if (t0 < 0.0f) t40 = t20 = t0 = n0 = gx0 = gy0 = 0.0f; /* No influence */
		...
		gradrot2(srdperm[ii + srdperm[jj]], sin_t, cos_t, &gx0, &gy0);
		t20 = t0 * t0;
		t40 = t20 * t20;
	...
	if (t1 < 0.0f) t21 = t41 = t1 = n1 = gx1 = gy1 = 0.0f; /* No influence */
		...
		gradrot2(srdperm[ii + i1 + srdperm[jj + j1]], sin_t, cos_t, &gx1, &gy1);
		t21 = t1 * t1;
		t41 = t21 * t21;
	...
	if (t2 < 0.0f) t42 = t22 = t2 = n2 = gx2 = gy2 = 0.0f; /* No influence */
		...
		gradrot2(srdperm[ii + 1 + srdperm[jj + 1]], sin_t, cos_t, &gx2, &gy2);
		t22 = t2 * t2;
		t42 = t22 * t22;
...
float SimplexRDNoise::SimplexRDNoise3D(float x, float y, float z, float angle,	float *dnoise_dx, float *dnoise_dy, float *dnoise_dz)
{
	...
	* Determine which simplex we are in. */
	...
	if (t0 < 0.0f) n0 = t0 = t20 = t40 = gx0 = gy0 = gz0 = 0.0f;
		...
		gradrot3(srdperm[ii + srdperm[jj + srdperm[kk]]], sin_t, cos_t, &gx0, &gy0, &gz0);
		t20 = t0 * t0;
		t40 = t20 * t20;
	...
	if (t1 < 0.0f) n1 = t1 = t21 = t41 = gx1 = gy1 = gz1 = 0.0f;
		...
		gradrot3(srdperm[ii + i1 + srdperm[jj + j1 + srdperm[kk + k1]]], sin_t, cos_t, &gx1, &gy1, &gz1);
		t21 = t1 * t1;
		t41 = t21 * t21;
	...
	if (t2 < 0.0f) n2 = t2 = t22 = t42 = gx2 = gy2 = gz2 = 0.0f;
		...
		gradrot3(srdperm[ii + i2 + srdperm[jj + j2 + srdperm[kk + k2]]], sin_t, cos_t, &gx2, &gy2, &gz2);
		t22 = t2 * t2;
		t42 = t22 * t22;
	...
	if (t3 < 0.0f) n3 = t3 = t23 = t43 = gx3 = gy3 = gz3 = 0.0f;
		...
		gradrot3(srdperm[ii + 1 + srdperm[jj + 1 + srdperm[kk + 1]]], sin_t, cos_t, &gx3, &gy3, &gz3);
		t23 = t3 * t3;
		t43 = t23 * t23;
...
float SimplexRDNoise::turbulence(float pnt[3], float lacunarity, float gain, int octaves, float flow)
{
	...
	{
		sum += amp * SimplexRDNoise3D(pp[0], pp[1], pp[2], flow, &dnoise_dx, &dnoise_dy, &dnoise_dz);
		amp *= gain;
...
void SimplexRDNoise::gradrot2(int hash, float sin_t, float cos_t, float *gx, float *gy) {
	int h = hash & 7;
	float gx0 = grad2[h][0];
	float gy0 = grad2[h][1];
...
void SimplexRDNoise::gradrot3(int hash, float sin_t, float cos_t, float *gx, float *gy, float *gz) {
	int h = hash & 15;
...
float SimplexRDNoise::graddotp2(float gx, float gy, float x, float y) {
	return gx * x + gy * y;
}

float SimplexRDNoise::graddotp3(float gx, float gy, float gz, float x, float y, float z) {
